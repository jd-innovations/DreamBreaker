-- match_recorded becomes catalog-driven.
--
-- The notification is written inside complete_personal_session_with_distribution,
-- which also creates guest shares and claim tokens. Only the notification
-- block changes; the rest is reproduced verbatim from the live definition,
-- including `#variable_conflict use_column`, which the returning query relies
-- on.
--
-- Copy is rendered ONCE rather than per row: every recipient of one session
-- gets the same sentence, and calling render_automation inside the insert's
-- select would run it per participant for no gain.
--
-- The catalog's seeded body promised {{opponent_name}} and {{score}}. Neither
-- exists here — a session has many participants and no single opponent, and
-- the score lives per game — so the copy says who recorded it and where, which
-- is what the original literal said.
--
-- Verified after applying: an edited catalog body came through the render path
-- with both variables substituted.

update public.notification_automations set
  title_template = 'Match recorded',
  body_template  = '{{recorder_name}} added your match at {{venue_name}}.',
  link_template  = '/(tabs)/stats',
  wired = true
where key = 'match_recorded';

CREATE OR REPLACE FUNCTION public.complete_personal_session_with_distribution(p_session_id uuid, p_facility_id uuid DEFAULT NULL::uuid, p_notes text DEFAULT NULL::text, p_indoor_outdoor text DEFAULT NULL::text)
 RETURNS TABLE(session_participant_id uuid, profile_id uuid, guest_player_id uuid, display_name text, phone text, participant_kind text, delivery_status text, guest_share_id uuid, claim_status text)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  -- Rendered once: every participant gets the same sentence.
  select * into v_copy from private.render_automation('match_recorded', jsonb_build_object(
    'recorder_name', coalesce(v_recorder_name, 'A player'),
    'venue_name',    coalesce(v_facility_name, 'a pickleball session')
  ));
  ...
  select
    ...
    'match_recorded',
    coalesce(v_copy.title, 'Match recorded'),
