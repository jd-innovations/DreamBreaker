-- =============================================================================
-- Fix direct DM insert RLS for legacy clients that omit conversation_type
-- =============================================================================
-- The mobile direct-message creator historically inserted only participant_a and
-- participant_b. Keep supporting that shape while preserving the contextual
-- conversation policy added for play events/tournaments.
...
comment on policy "conversations: participants insert" on public.conversations is
  'Direct: mutual Partner Finder like, tournament director<->registrant, play_event organizer<->claimed participant, or anyone<->play_event organizer. Contextual play_event/tournament conversations require a related event/tournament participant.';

-- Remove temporary debug RPCs used while diagnosing this RLS issue.
drop function if exists public.debug_dm_organizer_check(uuid);
drop function if exists public.debug_dm_full_check(uuid, uuid);
drop function if exists public.debug_show_conversations_policies();