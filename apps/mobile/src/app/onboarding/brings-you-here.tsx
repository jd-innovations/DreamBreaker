...
import { INTENT_OPTIONS } from '@/lib/onboarding/mockData';

const L = colors;
const PAGE_BG = '#F8F5EF';

export default function BringsYouHereScreen() {
  const insets = useSafeAreaInsets();
  const { draft, toggleInList } = useOnboarding();
  const canContinue = validators.intent(draft);
  const firstName = draft.firstName.trim();
  const title = firstName ? `Nice to meet you, ${firstName}.` : 'What brings you to PickleballApp?';
    ...
    <View style={[s.root, { paddingTop: insets.top + 8 }]}> 
      ...
      <View style={s.content}>
        <OnboardingEntrance style={s.titleBlock}>
          ...
          <Text style={s.subtitle}>What brings you to PickleballApp? Select all that apply.</Text>
        </OnboardingEntrance>
        ...
        <View style={s.grid}>
          {INTENT_OPTIONS.map((opt, index) => (
            <IntentCard
              ...
              label={opt.label}
              icon={opt.icon}
              selected={draft.intent.includes(opt.key)}
              onPress={() => toggleInList('intent', opt.key)}
            />
          ))}
        </View>
      </View>
      ...
      <View style={[s.footer, { paddingBottom: insets.bottom + 16 }]}> 
        <OnboardingCTA label="Continue" disabled={!canContinue} onPress={() => router.push('/onboarding/availability')} />
      </View>
    </View>
  );
}
