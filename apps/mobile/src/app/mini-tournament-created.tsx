...
import { getMiniTournament, updateCurrentPlayers, updateTournamentStatus, type MiniTournament } from '@/lib/miniTournamentStore';
import { getBracket, completedMatchCount } from '@/lib/bracketStore';
import {
  generateMiniTournamentBracket, fetchMiniTournamentMatches,
  deleteMiniTournamentBracket, sbMatchesToBracket, BracketExistsError,
...
// Maps a Supabase PlayEvent → the MiniTournament shape used by this screen.
// Bracket/format fields not stored in play_events use sensible defaults (Slice 1).
function playEventToMT(e: PlayEvent): MiniTournament {
  const status: MiniTournament['status'] =
  ...
  return {
    ...
    currentPlayers:   0,
    matchFormat:      '—',
    scoreTracking:    'Yes, track scores',
    warmupTime:       '—',
    visibility:       'public',
    notes:            e.notes ?? '',
...
const DEFAULT_TOURNAMENT: MiniTournament = {
  id: 'mt-demo',
  type: 'mini_tournament',
...
export default function MiniTournamentCreatedScreen() {
  ...
  // Resolve display object — must be defined before hooks that use g.id
  const localMT = getMiniTournament();
  const g: MiniTournament = isSupabase && sbEvent
    ? playEventToMT(sbEvent)
    : localMT ?? DEFAULT_TOURNAMENT;

  // ── Count + bracket state ──
  // Supabase: organizer auto-added as participant, start at 1.
  // Local: read from store.
  const [currentPlayers,    setCurrentPlayers]    = useState(() => isSupabase ? 1 : g.currentPlayers);
  const [showCelebration,   setShowCelebration]   = useState(false);
  const [participants,      setParticipants]      = useState<PlayParticipant[]>([]);
  const [sbBracketExists,   setSbBracketExists]   = useState(false);
  const [sbMatchesDone,     setSbMatchesDone]     = useState(0);
  const [generatingBracket, setGeneratingBracket] = useState(false);

  const bracketExists = isSupabase ? sbBracketExists : getBracket(g.id) !== null;
  const matchesDone   = isSupabase ? sbMatchesDone   : completedMatchCount(g.id);

  // ── Refresh counter — bumping triggers re-render which re-reads getMiniTournament() into g ──
  const [, setLocalRev] = useState(0);

  useFocusEffect(useCallback(() => {
    if (isSupabase) {
      // Use id! directly — always the real UUID, avoids g.id fallback issue
      Promise.all([
      ...
      if (latest) {
        setCurrentPlayers(latest.currentPlayers);
        setLocalRev(r => r + 1);
      }
    }
  }, [id, isSupabase]));

  const spotsLeft       = g.maxPlayers - currentPlayers;
  const canGenerate     = currentPlayers >= 4;
  ...
  const canJoin = isSupabase && !!user?.id && !myParticipant && !isOrganizer &&
    sbEvent?.status === 'open' && currentPlayers < (sbEvent?.max_players ?? 0);
  ...
  function handleMenuAction(id: string) {
    if (id === 'share')     { closeMenu(handleShare); return; }
    if (id === 'cancel')    { closeMenu(handleCancel); return; }
    if (id === 'invite')    { closeMenu(handleInvite); return; }
    if (id === 'duplicate') { closeMenu(() => comingSoon('Duplicate Tournament')); return; }
    if (id === 'edit')      { closeMenu(isOrganizer && sbEvent?.status === 'open' ? handleOpenEdit : () => comingSoon('Edit Tournament')); return; }
    closeMenu();
  }
  ...
  async function handleSaveEdit() {
    ...
    if (newMax < currentPlayers) {
      Alert.alert('Invalid', `Max players cannot be less than current roster (${currentPlayers}).`); return;
    }
    const trimName = editName.trim();
    if (!trimName) {
      Alert.alert('Invalid', 'Tournament name cannot be empty.'); return;
    }
    setEditSaving(true);
    try {
      ...
      if (msg === 'not_authenticated') Alert.alert('Error', 'You must be signed in to edit this tournament.');
      else if (msg === 'not_organizer') Alert.alert('Error', 'Only the organizer can edit this tournament.');
      else if (msg === 'event_not_open') Alert.alert('Error', 'Only open tournaments can be edited.');
      else if (msg === 'below_participant_count') Alert.alert('Invalid', `Max players cannot be less than current roster (${currentPlayers}).`);
  ...
  async function handleShare() {
    try {
      if (isSupabase && sbEvent) {
        ...
        await Share.share({
          message: `Join my mini tournament "${g.title}" on Pickleball App!\n${fmtDate(g.date)} at ${fmtTime(g.startTime)} — ${g.locationName}`,
          title: g.title,
        });
      }
    } catch { /* dismissed */ }
  ...
  async function handleGenerateBracket() {
    if (!canGenerate) {
      Alert.alert(
        'Not Enough Players',
        'You need at least 4 players registered before generating a bracket.',
        [{ text: 'Got It' }],
      );
      return;
    }
    ...
    if (isSupabase) {
      ...
      try {
        await generateMiniTournamentBracket(id!);
        setSbBracketExists(true);
        router.push(`/mini-tournament/${id!}/bracket` as never);
      } catch (err) {
        if (err instanceof BracketExistsError) {
          Alert.alert(
            'Bracket Already Exists',
            'A bracket was already generated for this tournament. Regenerate it?',
            [
              { text: 'Cancel', style: 'cancel' },
              {
                ...
                onPress: async () => {
                  setGeneratingBracket(true);
                  try {
                    await deleteMiniTournamentBracket(id!);
                    await generateMiniTournamentBracket(id!);
                    setSbBracketExists(true);
                    router.push(`/mini-tournament/${id!}/bracket` as never);
                  } catch {
                    Alert.alert('Error', 'Failed to regenerate bracket.');
          ...
          Alert.alert('Error', err instanceof Error ? err.message : 'Failed to generate bracket.');
        }
      } finally {
    ...
    router.push(`/mini-tournament/${g.id}/bracket` as never);
  }
        ...
        { text: 'Stay', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert('Error', 'Could not leave the tournament. Please try again.');
            }
          },
        },
      ],
  ...
  function handleMarkComplete() {
    Alert.alert(
      ...
      'This will close the tournament and move it to your completed events.',
      [
        { text: 'Not Yet', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert('Error', 'Could not complete the tournament. Please try again.');
            }
          },
        },
      ],
  ...
  return (
    <View style={s.root}>
      ...
      <View style={s.hero}>
        ...
        />
        <LinearGradient
          colors={['rgba(10,18,40,0.35)', 'transparent', 'rgba(10,18,40,0.80)']}
          style={StyleSheet.absoluteFill}
        />

        {/* floating nav */}
        <View style={[s.navRow, { paddingTop: insets.top + 8 }]}>
          <TouchableOpacity style={s.navBtn} onPress={() => router.replace('/(tabs)/games' as never)} activeOpacity={0.8}>
        ...
        <View style={s.heroPillRow}>
          <View style={s.heroPill}>
            <Ionicons name="trophy-outline" size={11} color="#fff" />
            <Text style={s.heroPillText}>MINI TOURNAMENT</Text>
          </View>
        </View>
        ...
        <View style={s.heroContent}>
          ...
          <View style={s.heroMeta}>
            ...
            <Text style={s.heroMetaText}>{fmtDate(g.date)} • {fmtTime(g.startTime)}</Text>
          </View>
          <View style={s.heroMeta}>
          ...
          <View style={s.heroMeta}>
            ...
            <Text style={s.heroMetaText}>{g.skillRange} Skill Level</Text>
          </View>
        </View>
      </View>
      ...
      >
        ...
        <View style={s.fillCard}>
          <View style={s.fillLeft}>
            ...
            <View>
              ...
              <Text style={s.fillSub}>
                {currentPlayers} of {g.maxPlayers} registered
                {'  '}
          ...
          <View style={s.fillDots}>
            {Array.from({ length: Math.min(g.maxPlayers, 16) }).map((_, i) => (
              <View
                key={i}
                style={[s.fillDot, i < currentPlayers ? s.fillDotFilled : s.fillDotEmpty]}
              />
            ))}
          </View>
          {g.maxPlayers > 16 && (
            <Text style={s.fillMore}>+{g.maxPlayers - 16} more spots</Text>
          )}
        </View>
          ...
          {isPastEvent && (
            <View style={s.pastNotice}>
              ...
              <Text style={s.pastNoticeText}>
                {isCancelled ? 'This tournament was cancelled — invites are closed.' : 'This tournament has ended — invites are closed.'}
              </Text>
            </View>
          )}
          <PrimaryButton
        ...
        <View style={s.card}>
          ...
          <View style={[s.playerRow, s.playerRowBorder]}>
            ...
            <View style={s.playerBody}>
              <Text style={s.playerName}>
                {organizer?.full_name ?? 'You'}{' '}
                <Text style={s.playerSub}>(Organizer)</Text>
              </Text>
              {organizer?.handle ? (
                <Text style={s.playerLevel}>@{organizer.handle}</Text>
              ) : (
                <Text style={s.playerLevel}>{g.skillRange}</Text>
              )}
            </View>
            <StatusChip label="Host" variant="green" />
          </View>

          <TouchableOpacity style={s.openSpotsRow} activeOpacity={0.75}>
          ...
          {/* Dev utility — fill roster with mock players for bracket testing (local events only) */}
          {currentPlayers < g.maxPlayers && !isSupabase && (
            <TouchableOpacity
              style={s.fillTestBtn}
              onPress={() => { updateCurrentPlayers(g.maxPlayers); setCurrentPlayers(g.maxPlayers); }}
              activeOpacity={0.8}
            >
              <Ionicons name="flask-outline" size={14} color={L.gold} />
              <Text style={s.fillTestText}>Fill with Test Players ({g.maxPlayers})</Text>
            </TouchableOpacity>
          )}
        </View>

        {/* 6 ── GENERATE BRACKET ── */}
        <View style={[s.generateCard, !canGenerate && s.generateCardDimmed]}>
          <View style={s.generateHeader}>
            <View style={s.generateIconWrap}>
              <Ionicons name="git-network-outline" size={20} color={L.gold} />
            </View>
            <View style={s.generateHeaderText}>
              <Text style={s.generateTitle}>Generate Bracket</Text>
              <Text style={s.generateSub}>
                {canGenerate
                  ? 'Create bracket or pool schedule from registered players.'
                  : `Need at least 4 players to generate bracket. ${4 - currentPlayers} more needed.`}
              </Text>
            </View>
          </View>
          <View style={s.generateMeta}>
            <View style={s.generateMetaItem}>
              <Ionicons name="trophy-outline" size={13} color={L.textSub} />
              <Text style={s.generateMetaText}>{g.tournamentType}</Text>
            </View>
            <View style={s.generateMetaItem}>
              <Ionicons name="people-outline" size={13} color={L.textSub} />
              <Text style={s.generateMetaText}>{currentPlayers}/{g.maxPlayers} registered</Text>
            </View>
          </View>
          <TouchableOpacity
            style={[s.generateBtn, (!canGenerate || generatingBracket) && s.generateBtnDisabled]}
            onPress={handleGenerateBracket}
            activeOpacity={(canGenerate && !generatingBracket) ? 0.85 : 0.5}
          >
            {generatingBracket ? (
              <ActivityIndicator size="small" color={L.white} />
            ) : (
              <Ionicons name="git-network-outline" size={17} color={L.white} />
            )}
        ...
        <View style={s.card}>
          <View style={s.sectionHeader}>
            <View style={s.sectionIconWrap}>
              <Ionicons name="git-network-outline" size={17} color={L.gold} />
            </View>
            <Text style={s.sectionTitle}>Bracket</Text>
            <TouchableOpacity style={s.viewAllBtn} onPress={() => router.push(`/mini-tournament/${g.id}/bracket` as never)} activeOpacity={0.7}>
              <Text style={s.viewAllText}>View</Text>
              <Ionicons name="chevron-forward" size={13} color={L.gold} />
            </TouchableOpacity>
          </View>
          <View style={s.emptyState}>
            <Ionicons name="git-network-outline" size={28} color={bracketExists ? L.gold : L.border} />
            <Text style={s.emptyText}>
              {bracketExists ? 'Bracket generated — tap View to open' : 'No bracket generated yet.'}
            </Text>
            {!bracketExists && <Text style={s.emptySub}>Generate bracket when players are ready.</Text>}
          </View>
        </View>
        ...
        <View style={s.card}>
          <View style={s.sectionHeader}>
            ...
            <Text style={s.sectionTitle}>Score Entry</Text>
            {bracketExists && (
              <TouchableOpacity
                style={s.viewAllBtn}
                activeOpacity={0.7}
                onPress={() => router.push(`/mini-tournament/${g.id}/bracket` as never)}
              >
                <Text style={s.viewAllText}>View</Text>
                <Ionicons name="chevron-forward" size={13} color={L.gold} />
              </TouchableOpacity>
          ...
          <View style={s.emptyState}>
            <Ionicons name="create-outline" size={28} color={bracketExists ? L.gold : L.border} />
            <Text style={s.emptyText}>
              {bracketExists
                ? `${matchesDone} match${matchesDone !== 1 ? 'es' : ''} completed — tap bracket to enter scores`
                : 'Scores will appear after bracket is generated.'}
            </Text>
          </View>
        </View>
        ...
        <View style={s.card}>
          <View style={s.sectionHeader}>
            ...
            {isCompleted && (
              <TouchableOpacity
                style={s.viewAllBtn}
                activeOpacity={0.7}
                onPress={() => router.push(`/mini-tournament/${g.id}/results` as never)}
              >
                <Text style={s.viewAllText}>View</Text>
                <Ionicons name="chevron-forward" size={13} color={L.gold} />
              </TouchableOpacity>
          ...
          <View style={s.emptyState}>
            <Ionicons name="podium-outline" size={28} color={isCompleted ? L.gold : L.border} />
            <Text style={s.emptyText}>
              {isCompleted
                ? 'Tournament complete — tap View to see results'
                : 'Results will appear after matches are completed.'}
            </Text>
          </View>
        </View>
        ...
        <View style={s.card}>
          <View style={s.chatHeader}>
            ...
            <View style={s.chatHeaderText}>
              <Text style={s.chatTitle}>Mini Tournament Chat</Text>
              <Text style={s.chatSub}>Message all players in this tournament</Text>
            </View>
          </View>
        ...
        <View style={[s.card, { padding: 0 }]}>
          ...
          <ActionRow
            ...
            sub="Edit details, format, or skill level"
            onPress={isOrganizer && sbEvent?.status === 'open' ? handleOpenEdit : () => comingSoon('Tournament Settings')}
            last
          />
        </View>

        {/* 12 ── CANCEL ── */}
        {/* JOIN TOURNAMENT — logged-in, not participant, not organizer, open + space */}
        {canJoin && (
          <TouchableOpacity style={s.joinCard} onPress={handleJoin} activeOpacity={0.8}>
            ...
            <View style={s.cancelBody}>
              <Text style={s.joinTitle}>Join Tournament</Text>
        ...
        {/* LEAVE TOURNAMENT — participant + not organizer + active only */}
        {canLeave && (
          <TouchableOpacity style={s.leaveCard} onPress={handleLeave} activeOpacity={0.8}>
            ...
            <View style={s.cancelBody}>
              <Text style={s.leaveTitle}>Leave Tournament</Text>
        ...
        {isSupabase && user?.id && sbEvent?.organizer_id === user.id &&
         g.status !== 'completed' && g.status !== 'cancelled' && (
          <TouchableOpacity style={s.completeCard} onPress={handleMarkComplete} activeOpacity={0.8}>
            ...
            <View style={s.cancelBody}>
              <Text style={s.completeTitle}>Mark Tournament as Complete</Text>
        ...
        <TouchableOpacity style={s.cancelCard} onPress={handleCancel} activeOpacity={0.8}>
          ...
          <View style={s.cancelBody}>
            <Text style={s.cancelTitle}>Cancel Mini Tournament</Text>
            <Text style={s.cancelSub}>This will remove the tournament for everyone.</Text>
          </View>
          <Ionicons name="chevron-forward" size={16} color={L.danger} />
        </TouchableOpacity>
        ...
        <>
          ...
          >
            ...
            >
              ...
              <Text style={[mm.rowLabel, mm.rowLabelDanger]}>Cancel Mini Tournament</Text>
            </TouchableOpacity>
          </Animated.View>
        </>
      )}
      ...
      <Modal visible={editOpen} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setEditOpen(false)}>
        <KeyboardAvoidingView style={s.modalRoot} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          <View style={s.modalHeader}>
            ...
            <Text style={s.modalTitle}>Edit Tournament</Text>
          ...
          <ScrollView style={s.modalScroll} contentContainerStyle={{ paddingBottom: 40 }} keyboardShouldPersistTaps="handled">
            ...
            <TextInput style={s.fieldInput} value={editName} onChangeText={setEditName} placeholder="e.g. Saturday Mini Tournament" placeholderTextColor={L.textSub} />
...
const s = StyleSheet.create({
  ...
  heroPillRow: { position: 'absolute', bottom: 140, left: 20 },
  heroPill: {
    flexDirection: 'row', alignItems: 'center', gap: 6,
    backgroundColor: '#DC2626',
    borderRadius: 20, paddingHorizontal: 14, paddingVertical: 6,
  },
  heroPillText: { fontSize: text.sectionLabel.size, fontWeight: '800', color: '#fff', letterSpacing: text.sectionLabel.letterSpacing },
  heroContent: { position: 'absolute', bottom: 20, left: 20, right: 20, gap: 4 },
  heroTitle: {
    color: L.white, fontSize: 32, fontWeight: '800', marginBottom: 4,
  ...
  scroll: { paddingHorizontal: spacing.screenH, paddingTop: 12, gap: 12 },
  ...
  fillTitle: { fontSize: text.rowValue.size, fontWeight: '800', color: L.navy },
  fillSub: { fontSize: text.caption.size, fontWeight: '500', color: L.textSub, marginTop: 2 },
  fillSpots: { fontWeight: '700', color: L.success },
  fillDots: { flexDirection: 'row', flexWrap: 'wrap', gap: 4 },
  fillDot: { width: 16, height: 6, borderRadius: 3 },
  ...
  nearbyBody: { flex: 1 },
  nearbyLabel: { fontSize: text.rowTitle.size, fontWeight: '700', color: L.navy },
  ...
  pastNoticeText: { fontSize: text.caption.size, fontWeight: '500', color: L.textSub, flex: 1 },
  ...
  rosterTitle: { fontSize: text.rowValue.size, fontWeight: '800', color: L.navy },
  viewAllBtn: { flexDirection: 'row', alignItems: 'center', gap: 2 },
  viewAllText: { fontSize: text.chipValue.size, fontWeight: '800', color: L.gold },
  ...
  playerBody: { flex: 1 },
  playerName: { fontSize: text.rowTitle.size, fontWeight: '700', color: L.navy },
  playerSub: { fontSize: text.caption.size, fontWeight: '500', color: L.textSub },
  playerLevel: { fontSize: text.caption.size, fontWeight: '500', color: L.textSub, marginTop: 1 },
  openSpotsRow: {
  ...
  openSpotsText: { fontSize: text.caption.size, fontWeight: '500', color: L.textSub },
  ...
  generateCardDimmed: { opacity: 0.75 },
  ...
  generateTitle: { fontSize: text.body.size, fontWeight: '500', color: L.navy },
  generateSub: { fontSize: text.caption.size, fontWeight: '500', color: L.textSub, marginTop: 2, lineHeight: 17 },
  generateMeta: { flexDirection: 'row', gap: 16 },
  generateMetaItem: { flexDirection: 'row', alignItems: 'center', gap: 5 },
  generateMetaText: { fontSize: text.caption.size, fontWeight: '500', color: L.textSub },
  generateBtn: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8,
    backgroundColor: L.navy, borderRadius: shape.cta, paddingVertical: 14,
  },
  generateBtnDisabled: { opacity: 0.45 },
  generateBtnText: { fontSize: text.actionLarge.size, fontWeight: '800', color: L.white },

  // 7-9 — Empty-state cards (bracket, score entry, results)
  sectionHeader: {
  ...
  sectionTitle: { fontSize: text.rowValue.size, fontWeight: '800', color: L.navy, flex: 1 },
  emptyState: {
  ...
  emptySub: { fontSize: text.caption.size, fontWeight: '500', color: L.textSub },
  ...
  chatTitle: { fontSize: text.rowValue.size, fontWeight: '800', color: L.navy },
  chatSub: { fontSize: text.caption.size, fontWeight: '500', color: L.textSub, marginTop: 1 },
  chatEmpty: {
    flexDirection: 'row', alignItems: 'center', gap: 10,
    backgroundColor: L.page, borderRadius: shape.cta,
    padding: 12, marginBottom: 10,
  },
  chatEmptyText: { fontSize: text.caption.size, fontWeight: '500', color: L.textSub, flex: 1, lineHeight: 17 },
  chatCta: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 4,
    borderWidth: 1.5, borderColor: L.goldBorder,
    borderRadius: shape.cta, paddingVertical: 10,
  },
  chatCtaText: { fontSize: text.action.size, fontWeight: '800', color: L.navy },
  ...
  joinTitle: { fontSize: text.rowTitle.size, fontWeight: '700', color: L.gold },
  joinSub: { fontSize: text.caption.size, fontWeight: '500', color: L.gold, opacity: 0.7, marginTop: 1 },
  ...
  leaveTitle: { fontSize: text.rowTitle.size, fontWeight: '700', color: L.danger },
  leaveSub: { fontSize: text.caption.size, fontWeight: '500', color: L.danger, opacity: 0.7, marginTop: 1 },
  ...
  completeTitle: { fontSize: text.rowTitle.size, fontWeight: '700', color: colors.success },
  completeSub: { fontSize: text.caption.size, fontWeight: '500', color: colors.success, opacity: 0.7, marginTop: 1 },
  ...
  cancelBody: { flex: 1 },
  cancelTitle: { fontSize: text.rowTitle.size, fontWeight: '700', color: colors.danger },
  cancelSub: { fontSize: text.caption.size, fontWeight: '500', color: colors.danger, opacity: 0.7, marginTop: 1 },
  ...
  modalTitle: { fontSize: text.modalTitle.size, fontWeight: '900', color: L.navy },
  modalCancel: { fontSize: text.body.size, fontWeight: '500', color: L.textSub },
  modalSave: { fontSize: text.body.size, fontWeight: '500', color: L.gold },
  modalScroll: { flex: 1, paddingHorizontal: 20, paddingTop: 20 },
  fieldLabel: { fontSize: text.sectionLabel.size, fontWeight: '800', color: L.textSub, letterSpacing: text.sectionLabel.letterSpacing, textTransform: 'uppercase', marginBottom: 6, marginTop: 16 },
  fieldInput: { backgroundColor: L.bg, borderWidth: 1, borderColor: L.border, borderRadius: shape.card, paddingHorizontal: 14, paddingVertical: 11, fontSize: text.body.size, fontWeight: '500', color: L.navy },
  ...
  fillTestText: { fontSize: text.link.size, fontWeight: '700', color: L.navy },
});
