...
import { ArrowRight, Plus, Heart, Lightning, Trophy, Users, MapPin, Calendar } from "@phosphor-icons/react";
import { Button } from "../components/ui/button";
import { PageShell } from "../components/PageShell";
import { HERO_IMG, tournaments } from "../data/mockData";
...
const features = [
  {
    ...
    body: "Pay a small refundable fee to lock your tournament entry. Confirm later, no scramble.",
  },
  {
    icon: Heart,
    tag: "MATCHMAKING",
    title: "Tinder-style partner finder",
    body: "Swipe through verified players by DUPR, distance and play style. Match. Compete.",
  },
  {
    icon: Trophy,
    tag: "BRACKETS",
    title: "Live brackets & rankings",
    body: "Auto-generated draws. Real-time scoring. Auto-updated DUPR after every event.",
  },
];
          ...
          <div className="max-w-3xl">
            ...
            <div className="mt-9 flex flex-col sm:flex-row gap-3 max-w-xl">
              <Link to="/tournaments" className="flex-1">
                <Button
                  className="w-full rounded-full h-14 bg-secondary/80 text-foreground hover:bg-secondary border border-border text-sm font-display tracking-[0.2em] justify-between px-7"
                  data-testid="hero-explore-btn"
                >
                  EXPLORE TOURNAMENTS <ArrowRight size={18} weight="bold" />
                </Button>
              </Link>
              <Link to="/director" className="flex-1">
                <Button
                  className="w-full rounded-full h-14 bg-primary text-primary-foreground hover:bg-primary/90 text-sm font-display tracking-[0.2em] justify-between px-7"
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <div className="flex items-end justify-between flex-wrap gap-4 mb-10">
          <div>
            ...
            <h2 className="font-display text-4xl sm:text-5xl lg:text-6xl tracking-wide max-w-xl">
              BUILT FOR PLAYERS<br />WHO CAME TO WIN.
            </h2>
          </div>
          <p className="text-sm text-muted-foreground max-w-sm">
            Three tools that change how you compete. No more spreadsheets, no more {`"DM for partner"`} posts, no more missed spots.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {features.map((f, i) => (
            <div
              ...
              className="border border-border rounded-2xl p-7 bg-card hover:border-primary transition-all duration-300 group"
            >
      ...
      <section className="border-t border-border bg-card/40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
          <div className="flex items-end justify-between flex-wrap gap-4 mb-10">
            ...
            <Link
              to="/tournaments"
              className="font-display tracking-[0.2em] text-sm flex items-center gap-2 text-primary hover:gap-3 transition-all"
              data-testid="featured-viewall-link"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {tournaments.slice(0, 3).map((t) => (
              <Link
                ...
                className="group border border-border rounded-2xl overflow-hidden bg-card hover:border-primary transition-all"
              >
                <div className="relative h-44 overflow-hidden">
                  ...
                  <div className="absolute top-3 left-3 px-2.5 py-1 rounded-full bg-primary text-primary-foreground text-[10px] font-mono tracking-widest font-bold">
                    {t.status.toUpperCase()}
                  </div>
                  <div className="absolute bottom-3 left-3 right-3">
                    <div className="font-display text-2xl text-white tracking-wide">{t.name}</div>
                  </div>
                </div>
                <div className="p-5 space-y-2">
                  ...
                  <div className="flex items-center gap-2 text-sm text-muted-foreground">
                    ...
                    <span>{t.filled} / {t.spots} players</span>
                  </div>
        ...
        <div className="border border-border rounded-3xl p-10 lg:p-16 bg-card relative overflow-hidden">
          ...
          <div className="relative grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
            <div>
              ...
              <h2 className="font-display text-5xl lg:text-6xl tracking-wide leading-[0.95]">
                YOUR NEXT TROPHY<br />IS ONE SWIPE AWAY.
              </h2>
            </div>
            <div className="flex flex-col sm:flex-row gap-3 lg:justify-end">
              <Link to="/matchmaking">
                <Button className="rounded-full h-14 px-8 bg-primary text-primary-foreground hover:bg-primary/90 font-display tracking-[0.2em]" data-testid="cta-matchmaking-btn">
                  FIND PARTNER
                </Button>
              </Link>
              <Link to="/auth?mode=signup">
                <Button variant="ghost" className="rounded-full h-14 px-8 border border-border font-display tracking-[0.2em]" data-testid="cta-signup-btn">
                  CREATE ACCOUNT
                </Button>
              </Link>
            </div>
