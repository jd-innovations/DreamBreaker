...
export const HoldMySpotDialog = ({ open, onOpenChange, tournament }) => {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) setStep("review"); onOpenChange(v); }}>
      <DialogContent className="max-w-md border-border" data-testid="hold-spot-dialog">
        {step === "review" && (
          <>
            <DialogHeader>
              ...
              <DialogDescription className="text-muted-foreground">
                {tournament.location} · {tournament.date} · {tournament.format}
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-2">
              <input
                data-testid="hold-card-number"
                value={card.number}
                onChange={(e) => setCard({ ...card, number: e.target.value })}
                placeholder="Card number  ·  4242 4242 4242 4242"
                className="w-full bg-secondary border border-border rounded-md px-4 py-3 text-sm font-mono outline-none focus:ring-2 focus:ring-primary"
              />
              <div className="grid grid-cols-2 gap-2">
                <input
                  data-testid="hold-card-exp"
                  value={card.exp}
        ...
        {step === "success" && (
          <div className="text-center py-4" data-testid="hold-success-state">
            ...
            <Button
              onClick={reset}
              className="rounded-full bg-primary text-primary-foreground mt-6 px-6"
              data-testid="hold-done-btn"
            >
              Done
            </Button>
          </div>
