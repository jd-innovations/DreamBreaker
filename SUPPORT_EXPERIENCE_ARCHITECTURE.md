...
- **Collision resolution order**: (1) screen-declared sticky bottom bar → button sits above it; (2) tab bar → `tabBarClearance()`; (3) screen's own floating control at the same corner (e.g. `tournament/[id]` photo badge) → button offsets vertically above it; (4) default corner position.
