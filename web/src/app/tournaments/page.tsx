              ...
              return (
                <div key={t.id} data-testid={`tournament-card-${t.id}`} className="group border border-border rounded-2xl overflow-hidden bg-card hover:border-primary transition-all flex flex-col">
                  <Link href={`/tournaments/${t.id}`} className="relative h-44 overflow-hidden block">
                    {/* coverImg traces back to tournaments.cover_img_url,
                        free-text a director can set to anything -- SafeImage,
                        not Image directly. See lib/image-hosts.ts. */}
                    <SafeImage src={coverImg} alt="" fill sizes="(min-width: 1024px) 33vw, (min-width: 768px) 50vw, 100vw" className="object-cover group-hover:scale-105 transition-transform duration-500" />
